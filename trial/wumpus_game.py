...
class WumpusGame:
    ...
    def move_character(self, dx, dy):
        ...
        if 0 <= new_x < BOARD_WIDTH and 0 <= new_y < BOARD_HEIGHT:
            if tuple([new_x, new_y]) in self.wumpus_positions:
                print("You encountered a wumpus! Game Over!")
                self.game_over = True
            elif tuple([new_x, new_y]) in self.pit_positions:
        ...
        while running:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    elif tuple(self.char_pos) in self.wumpus_positions:
                        print("You encountered a wumpus! Game Over!")
                        running = False
                        break
                    
                    elif tuple(self.char_pos) in self.pit_positions:
